...
@pytest.fixture
def mock_bsn_update_scheme(mock_bsn_number: str, mock_referral: ReferralEntity) -> BsnUpdateScheme:
...
def test_synchronize_should_succeed_when_there_is_data_from_metadata(
    ...
    mock_pseudonym_register.return_value = mock_pseudonym_jwe
    mock_nvi_get_referrals.return_value = None
    expected = mock_update_scheme

    actual = synchronizer.synchronize(DataDomain("ImagingStudy"), mock_domain_map_entry)
...
def test_synchronize_should_succeed_and_update_timestamp_on_domain_entry_when_there_is_update_from_metadata(
    ...
    actual = synchronizer.synchronize(DataDomain("ImagingStudy"), mock_domain_map_entry_with_timestamp)
...
def test_synchronize_should_fail_when_nvi_is_unreachable(
    ...
    with pytest.raises(ConnectionError):
        synchronizer.synchronize(DataDomain("ImagingStudy"), mock_domain_map_entry)
...
def test_synchronize_domain_should_succeed_when_there_is_data_to_update(
    ...
    actual = synchronizer.synchronize_domain(DataDomain("ImagingStudy"))
...
def test_synchronize_should_fail_when_there_is_no_connection_established(
    ...
    with pytest.raises(ConnectionError):
        synchronizer.synchronize_domain(DataDomain("ImagingStudy"))
