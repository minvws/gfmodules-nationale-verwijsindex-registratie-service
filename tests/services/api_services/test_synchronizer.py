...
PATCHED_NVI_API = "app.services.api.nvi_api_service.NviApiService"
...
def test_synchronize_should_succeed_when_there_is_data_from_metadata(
    ...
    mock_healthcheck.return_value = {"nvi_api": True, "metadata_api": True, "pseudonym_api": True}
    ...
    expected = UpdateScheme(
        updated_data=[
            BsnUpdateScheme(
                ...
                referral=Referral(pseudonym="some_pseudonym", data_domain="beeldbank", ura_number="12345678"),
            )
        ],
        domain_entry=DomainMapEntry(
            resource_type="ImagingStudy", last_resource_update=mock_update_scheme_with_new_timestamp[1]
        ),
    )
