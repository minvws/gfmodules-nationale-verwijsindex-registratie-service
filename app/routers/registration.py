...
from app.data import BSN, DataDomain, UraNumber
...
def validate_careplan(resource: Dict[str, Any]) -> Tuple[Resource, DataDomain, BSN]:
    ...
    return careplan, DataDomain.CarePlan, bsn
...
def create(
    ...
    (_, data_domain, bsn) = validate_careplan(request)

    ura = UraNumber(get_config().app.ura_number)
    local_pseudonym = pseudonym_service.exchange_for_bsn(bsn)

    referral_service.create_referral(local_pseudonym, data_domain, ura)
